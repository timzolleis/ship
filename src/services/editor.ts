...
export class EditorService extends Context.Tag("ship/EditorService")<EditorService, EditorShape>() {
  static layer: Layer.Layer<
    EditorService,
    never,
    ShellService | ConfigService | FileSystem.FileSystem
  > = Layer.effect(
    EditorService,
    Effect.gen(function* () {
  ...
  static layerMemory: (opened?: Ref.Ref<ReadonlyArray<string>>) => Layer.Layer<EditorService> = (opened) =>
    Layer.succeed(EditorService, {
      open: (path) =>
        opened ? Ref.update(opened, (xs) => [...xs, path]) : Effect.void,
    })
}
